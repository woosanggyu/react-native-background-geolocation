...
/**
 * Default config merged in `configure`.
 * NOTE: `useSignificantChanges` is iOS-only (native patch required).
 */
var DEFAULT_CONFIG = {
  // 기존 기본값이 있다면 여기로 옮겨서 관리하세요.
  // 예: desiredAccuracy, distanceFilter 등
...
var BackgroundGeolocation = {
  ...
  configure: function (config, successFn, errorFn) {
    ...
    // 새 기본값 병합 (스프레드가 없을 수도 있어 Object.assign 사용)
    var merged = Object.assign({}, DEFAULT_CONFIG, config || {});
    RNBackgroundGeolocation.configure(merged, successFn, errorFn);
  },
  ...
  getLogEntries: function (
    limit /*, offset = 0, minLevel = "DEBUG", successFn = emptyFn, errorFn = emptyFn */
  ) {
    ...
    if (acnt > 1 && typeof arguments[1] == "function") {
      ...
      console.log(
        "[WARN]: Calling deprecated variant of getLogEntries method."
      );
      offset = 0;
      ...
      minLevel =
        acnt > 2 && arguments[2] !== undefined
          ? arguments[2]
          : BackgroundGeolocation.LOG_DEBUG;
      successFn =
        acnt > 3 && arguments[3] !== undefined ? arguments[3] : emptyFn;
      errorFn = acnt > 4 && arguments[4] !== undefined ? arguments[4] : emptyFn;
    }

    RNBackgroundGeolocation.getLogEntries(
      limit,
      offset,
      minLevel,
      successFn,
  ...
  removeAllListeners: function (event) {
    ...
    if (this.events.indexOf(event) < 0) {
      console.log(
        "[WARN] " +
          TAG +
          ': removeAllListeners for unknown event "' +
          event +
          '"'
      );
    } else {
